...
번역(kind=역) 인용문은 literature-clock 데이터셋에서 유래했다.
출처: https://github.com/JohannesNE/literature-clock (CC BY-NC-SA 2.5,
https://creativecommons.org/licenses/by-nc-sa/2.5/). 한국어 번역과 분 단위
재구성을 거쳤으므로 원본을 변경한 파생물이다. 그 데이터에 없던 8개 시각은
cdmoro 등 literary-clock fork와 원문 대조로 보완했다.

번역 대상이 된 현대 문학 작품의 발췌문 자체는 각 저자와 출판사의 저작권 아래 있다.
위 CC 라이선스는 데이터셋 편집물에 적용되며 원작의 권리를 대체하지 않는다.
...
- **코드**: MIT. `LICENSE` 참고.
- **원문(kind=원문) 인용문**: 저작권 만료 작품 발췌, 퍼블릭 도메인.
- **번역(kind=역) 인용문 데이터셋**: CC BY-NC-SA 2.5. literature-clock 데이터의
  파생물이라 동일 조건으로 배포하며 비영리 이용에 한한다. 개별 발췌문의 원저작권은
  각 저자와 출판사에 있다.
- **폰트, 스프라이트**: 이 저장소에 포함하지 않는다(gitignore). 빌드 시 각자 준비하며
  해당 라이선스를 따른다.
